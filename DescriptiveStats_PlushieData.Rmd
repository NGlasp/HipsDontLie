...
assign("gor1", c(37,	123,	20,	178,	21,	17.5,	6.5,	22))
...
range.flex
#Answer: 11 degrees
range.extend <- max(plush.dat$Extension) - min(plush.dat$Extension)
range.extend
range.add <- max(plush.dat$Adduction) - min(plush.dat$Adduction)
range.add
range.abd <- max(plush.dat$Abduction) - min(plush.dat$Abduction)
range.abd
range.Pelbread <- max(plush.dat$PelvicBreadth) - min(plush.dat$PelvicBreadth)
range.Pelbread
...
IQR_Flex <- IQR(plush.dat$Flexion)
IQR_Ext <- IQR(plush.dat$Extension)
IQR_Add <- IQR(plush.dat$Adduction)
IQR_Abd <- IQR(plush.dat$Abduction)
IQR_PB <- IQR(plush.dat$PelvicBreadth)
IQR_MTC <- IQR(plush.dat$MidthighCircumference)
...
SD_Flex <- sd(plush.dat$Flexion)
SD_Ext <- sd(plush.dat$Extension)
SD_Add <- sd(plush.dat$Adduction)
SD_Abd <- sd(plush.dat$Abduction)
SD_PB <- sd(plush.dat$PelvicBreadth)
SD_MTC <- sd(plush.dat$MidthighCircumference)
...
Avg_Ext <- mean(plush.dat$Extension)
Avg_Add <- mean(plush.dat$Adduction)
Avg_Abd <- mean(plush.dat$Abduction)
Avg_PB <- mean(plush.dat$PelvicBreadth)
Avg_MTC <- mean(plush.dat$MidthighCircumference)
Avg_Flex <- mean(plush.dat$Flexion)


# Can you combine these mean vectors into a matrix? 
means <- c(Avg_Ext, Avg_Add, Avg_Abd, Avg_PB, Avg_MTC, Avg_Flex)



# Can you make vectors and matricies for the other statistics that we calculated above?
Standard_Deviants <- c(SD_Flex, SD_Ext, SD_Add, SD_Abd, SD_PB, SD_MTC)
Ranges <- c(range.extend, range.add, range.abd, range.Pelbread, range.flex)
IQR <- c(IQR_Flex,IQR_Ext, IQR_Add, IQR_Abd, IQR_PB, IQR_MTC )
